...
namespace NS_Composants { 
    ...
    String^ CL_mappTBADRESSE::SELECTByIdPersonne(void) { 
        return "SELECT id_adresse, adresse, ville, cp, id_personne " + "FROM TB_ADRESSE " + "WHERE(id_personne = " + this->id_personne + ");"; 
    }

    String^ CL_mappTBADRESSE::INSERT(void) { 
        return "INSERT INTO TB_ADRESSE(adresse, ville, cp, id_personne) " + "VALUES('" + this->adresse + "', '" + this->ville + "', '" + this->cp + "', " + this->id_personne + ");"; 
    }

    String^ CL_mappTBADRESSE::UPDATE(void) { 
        return "UPDATE TB_ADRESSE " + "SET adresse ='" + this->adresse + "', ville ='" + this->ville + "', cp ='" + this->cp + "' " + "WHERE(id_adresse =  " + this->id_adresse + "); "; 
    }

    String^ CL_mappTBADRESSE::DELETE(void) { 
        return "DELETE FROM TB_ADRESSE " + "WHERE(id_adresse = " + this->id_adresse + ");"; 
    }
